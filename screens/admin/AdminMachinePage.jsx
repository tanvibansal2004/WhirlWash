...
const AdminMachinePage = () => {
    ...
    const checkAdminStatus = async () => {
      if (currentUser) {
        try {
          ...
          const snapshot = await adminRef.where('email', '==', currentUser.email).get();
  ...
  const handleAddMachine = async () => {
    try {
      ...
      Alert.alert('Success', `Machine No. ${newMachineNumber} added successfully.`);
    } catch (error) {
  ...
  const handleUpdateMachine = async () => {
    try {
      ...
      const updateData = {
        underMaintenance: underMaintenance,
        status: underMaintenance ? 'maintenance' : currentMachine.inUse ? 'in-use' : 'available',
      };
      ...
      await firestore().collection('machines').doc(currentMachine.id).update(updateData);
      
      setEditModalVisible(false);
      
      const statusChangeMsg = prevStatus !== newStatus 
        ? `Machine No. ${currentMachine.number} ${underMaintenance ? 'marked as under maintenance' : 'marked as operational'}.`
            ...
            machines.map(machine => (
              <View key={machine.id} style={styles.machineCard}>
                <View style={styles.machineHeader}>
                  <Text style={styles.machineTitle}>Machine No. {machine.number}</Text>
                  ...
                  {machine.inUse && (
                    <>
                      <View style={styles.detailRow}>
                        ...
                        <Text style={styles.detailValue}>{machine.bookedBy || 'Unknown'}</Text>
                      </View>
                      
                      {machine.userName && (
                        <View style={styles.detailRow}>
                          <Text style={styles.detailLabel}>User Name:</Text>
                          <Text style={styles.detailValue}>{machine.userName}</Text>
                        </View>
                      )}
                      ...
                      {machine.userMobile && (
                        <View style={styles.detailRow}>
                          ...
                          <Text style={styles.detailValue}>{machine.userMobile}</Text>
                        </View>
                      )}
                  ...
                  {!machine.inUse && machine.lastUsedBy && (
                    <>
                      <View style={styles.detailRow}>
                        ...
                        <Text style={styles.detailValue}>{machine.lastUsedBy}</Text>
                      </View>
                      
                      {machine.lastUserName && (
                        <View style={styles.detailRow}>
                          <Text style={styles.detailLabel}>User Name:</Text>
                          <Text style={styles.detailValue}>{machine.lastUserName}</Text>
                        </View>
                      )}
                      ...
                      {machine.lastUserMobile && (
                        <View style={styles.detailRow}>
                          <Text style={styles.detailLabel}>Mobile:</Text>
                          <Text style={styles.detailValue}>{machine.lastUserMobile}</Text>
                        </View>
                      )}
